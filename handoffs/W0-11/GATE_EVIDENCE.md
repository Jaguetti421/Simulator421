...
| A real 3D screenshot of eight procedural identities is produced through /capture under Playwright with metadata | `handoffs/W0-10/evidence/identity-kit-prototype8.png`, `capture-t300.png`, `capture-metadata.txt` | Chromium 1194, **software WebGL (SwiftShader)**, 960×540 | **PASS (artifact) / HUMAN_REQUIRED (judgement)** — both PNGs carry fixture, tick, camera, kernel, authoritative digest and `Watermark=FakeSim` in `tEXt`. The scene reports 39 instances for the eight recipes (24 body, 8 headwear, 7 accessories). **Whether the eight read as distinguishable characters to a person is not certified here** — see JANI_PC_CHECKLIST.md step 2 |
...
Real human/reference-machine observations and who supplied them: **none yet.** Nothing in this gate has been seen on real hardware or judged by a person. Every visual artifact was produced with software rendering; every timing number came from a 1-CPU sandbox. `JANI_PC_CHECKLIST.md` is the list of what only Jani can supply, and `PLAYTEST_BRIEF.md` is the viewing script.
