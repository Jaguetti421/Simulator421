...
- **Token hygiene:** the token was pasted in chat; Jani may rotate it at any time — the developer needs it only per session. CI on the GitHub runner has not run yet (first run triggers on this push; check the Actions tab).
